...
┌─────────────────────────────────────────────────────┐
│                  WORDPRESS CORE                     │
│  (Routing, Database, Authentication, APIs)          │
│              ⚠️ NEVER MODIFY                       │
└────────────────────┬────────────────────────────────┘
                     │
         ┌───────────┴───────────┐
...
│                 │    │                     │
│ • Data models   │    │ • HTML templates    │
│ • Business      │    │ • CSS styling       │
│   logic         │    │ • JavaScript UI     │
│ • Integrations  │    │ • Display logic     │
│ • Workflows     │    │   ONLY              │
└─────────────────┘    └─────────────────────┘
```
...
**Maintainability:** Clear boundaries reduce complexity. Developers know exactly where to implement features—business logic goes in plugins, presentation goes in themes.

### 2.4 Correct Implementation

**❌ INCORRECT - Logic in Theme:**
...
**✅ CORRECT - Logic in Plugin:**

```php
// Plugin: ecommerce-payment-handler/payment-handler.php
// ✅ GOOD: Business logic in plugin
function process_payment($order_id) {
    $order = get_post($order_id);
    $stripe = new StripeAPI();
    $charge = $stripe->charge([
...
// ✅ GOOD: Theme only handles display
<?php get_header(); ?>
...
│              registered to 'init' hook        │
│                                               │
│  Plugin A: add_action('init', 'funcA', 10);   │
│  Plugin B: add_action('init', 'funcB', 20);   │
│  Plugin C: add_action('init', 'funcC', 5);    │
│                                               │
│  Execution order (by priority):               │
│  1. funcC (priority 5)                        │
│  2. funcA (priority 10, default)              │
│  3. funcB (priority 20)                       │
│                                               │
└───────────────────────────────────────────────┘
```
...
function add_estimated_reading_time($content) {
    ...
    $reading_notice = sprintf(
        '<div class="reading-time" style="padding: 10px; background: #f0f0f0; margin-bottom: 20px;">
            <strong>⏱ Estimated reading time:</strong> %d %s
        </div>',
        $reading_time,
...
// After all filters: <span class="custom-title">📝 HELLO WORLD</span>
```
...
wp_loaded           → All PHP files loaded
```

**Query Hooks:**

```
pre_get_posts       → Before main query runs
posts_selection     → After posts retrieved
the_post            → During loop iteration
loop_start          → Beginning of loop
...
**Never Check Roles—Always Check Capabilities:**

```php
// ❌ INCORRECT: Checking user role
...
1. Never removing functions—only deprecating them
...
Legacy plugin from 2011 → Still works in 2023
...
get_currentuserinfo();  // ✓ Works but triggers notice in debug mode
````
...
```php
// ❌ OLD (Deprecated in WordPress 4.5)
...
// Old code works
$posts = get_posts(['numberposts' => 5]);  // ✓ Still works
...
```php
// ✅ GOOD: Check if function exists before using
...
// ✅ GOOD: Use constants that handle version differences
...
- ✓ Safe upgrades for production sites
...
- ✗ Accumulated technical debt in core
- ✗ Larger codebase maintaining legacy code
- ✗ Slower adoption of modern PHP features
...
// Total queries: 1 (initial) + (100 × 3) = 301 queries
```

**✅ EFFICIENT: Optimized with Proper Caching**
...
foreach ($posts as $post) {
    // These now use cached data—no additional queries
...
```
Request → Nginx/Varnish → Cached HTML (served immediately)
   ↓
   └─► If cache miss, generate page and cache result
```
...
the_post_thumbnail('product-thumb');  // ✅ Optimized
?>
```
...
│                                              │
│  ┌──────────────┐    ┌──────────────────┐  │
│  │ Core Team    │    │  Plugin Authors  │  │
│  │ (Leadership) │    │  (60,000+ plugins)│  │
│  └──────────────┘    └──────────────────┘  │
│                                              │
│  ┌──────────────┐    ┌──────────────────┐  │
│  │ Contributors │    │  Theme Designers │  │
│  │ (Global)     │    │  (11,000+ themes)│  │
│  └──────────────┘    └──────────────────┘  │
│                                              │
│  ┌──────────────────────────────────────┐  │
│  │      Documentation Writers           │  │
│  │      Translators (200+ languages)    │  │
...
```php
// ✅ CORRECT: WordPress coding style
...
├── Core translations (200+ languages)
├── Plugin translations
├── Theme translations
...
├── Code Reference (auto-generated from inline docs)
├── Plugin Handbook
├── Theme Handbook
├── REST API Handbook
...
├── WordCamp US (flagship event)
├── WordCamp Europe
├── WordCamp Asia
└── 100+ city-specific WordCamps

WordPress Meetups
└── 750+ local meetup groups worldwide
```
...
│  Enables long-term stability for all layers   │
└───────────────┬──────────────────────────────┘
                │
    ┌───────────┴───────────┐
    │                       │
...
// ✅ CORRECT: Logic in plugin, display in theme
...
These principles are not theoretical—they are proven patterns that have scaled WordPress from a simple blogging tool to an enterprise-grade application platform. By understanding and applying these principles, developers can:
...
**Section:** 1.2 Core Principles — Foundational Engineering Philosophy  
